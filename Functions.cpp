...
void printCap(int op) {// 1 - рейс, 2 - тур/экскурсия
	...
	if (op == 2) {
		...
		cout << setw(13) << "Отправление" << "|" << setw(19) << "В пути" << "|" << setw(12) <<
	...
	while (flag) {
		...
		try {
			...
			for (int i = 0; i < text.size(); i++) {
				if (op == 1) {
					if ((text[i] < 'а' || text[i] > 'я') && (text[i] < 'А' || text[i] > 'Я')) throw BadInput(-21);
				}
				if (op == 2) {
					if ((text[i] < 'а' || text[i] > 'я') && (text[i] < 'А' || text[i] > 'Я') && text[i] != ' ') throw BadInput(-27);
				}
			}
		}
		catch (BadInput& obj) {
...
string EnterTripDate() {
	cout << "Просмотр рейсов возможен только до фефраля 2020 г.!\n";
